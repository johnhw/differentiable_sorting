...
The rest of the code is simply computing the `l, r, l_inv, r_inv` matrices, which are fixed for a given `n`. If we're willing to include a split and join operation, we can reduce this to a single `n x n` multiply for each layer (plus one extra multiply at the start) by fusing consecutive permutations:
...
```python
    def diff_sort_weave(fused, x, softmax=np.maximum):
        ...
        for mat in fused[1:]:
            a, b = x[:split], x[split:]
            mx = np.log(np.exp(a) + np.exp(b))
            x = mat @ np.concatenate([a + b - mx, mx])
        return x
```
