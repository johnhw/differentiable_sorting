...
### Softmax (log-sum-exp)
def softmax(a, b, alpha=1):
    """The softmaximum of softmax(a,b) = log(e^a + a^b)."""
    return np.log(np.exp(a * alpha) + np.exp(b * alpha)) / alpha


def softmin(a, b, alpha=1):
    """
    Return the soft-minimum of a and b
    The soft-minimum can be derived directly from softmax(a,b).
    """
    return a + b - softmax(a, b, alpha)


def softcswap(a, b, alpha=1):
    """Return a,b in 'soft-sorted' order, with the smaller value first"""
    return softmin(a, b, alpha), softmax(a, b, alpha)


### Smooth max
def smoothmax(a, b, alpha=20):
    return (a * np.exp(a * alpha) + b * np.exp(b * alpha)) / (
        np.exp(a * alpha) + np.exp(b * alpha)
    )


def smoothmin(a, b, alpha=1):
    """
    Return the soft-minimum of a and b
    The soft-minimum can be derived directly from softmax(a,b).
    """
    return a + b - smoothmax(a, b, alpha)


def smoothcswap(a, b, alpha=1):
    """Return a,b in 'soft-sorted' order, with the smaller value first"""
    return smoothmin(a, b, alpha), smoothmax(a, b, alpha)
...
def diff_bisort(matrices, x, alpha=1.0):
    """
    ...
    """
    for l, r, map_l, map_r in matrices:
        a, b = softcswap(l @ x, r @ x, alpha=alpha)
        x = map_l @ a + map_r @ b

    return x


def smoothmax_bisort(matrices, x, alpha=1):
    """
    Approximate differentiable sort. Takes a set of bitonic sort matrices generated by bitonic_matrices(n), sort 
    a sequence x of length n. Values may be distorted slightly but will be ordered.
    Uses the smoothmax function instead of logsumexp
    """
    for l, r, map_l, map_r in matrices:
        a, b = smoothcswap(l @ x, r @ x, alpha)
        x = map_l @ a + map_r @ b
...
def bisort_weave(matrices, x):
    """
    Given a set of woven bitonic sort matrices generated by bitonic_woven_matrices(n), sort 
    a sequence x of length n. Requires splitting and joining matrices, but fewer multiplies.
    """
    split = len(x) // 2
    for weave, unweave in matrices:
        ...
        x = unweave @ np.hstack(
            [
                np.minimum(woven[:split], woven[split:]),
                np.maximum(woven[:split], woven[split:]),
            ]
        )
    return x
